...
function filesContaining(value: string): string[] {
  try {
    const out = execFileSync("grep", ["-rlaF", "--", value, ROOT], { encoding: "utf8" });
    return out.split("\n").filter(Boolean);
  } catch {
...
for (const [name, value] of values) {
  ...
  const hits = filesContaining(value);
  if (hits.length > 0) leaked.push(`  ${name}: ${hits.length} file(s), first: ${hits[0]}`);
}

if (leaked.length > 0) {
  console.error(
    `check-build-secrets: a secret's value is in ${ROOT}.\n${leaked.join("\n")}\n\n` +
...
console.log(
  `check-build-secrets: control fired, ${values.size} secret value(s) checked, none in ${ROOT}.`,
);
