...
be clicked, and a board of scattered single pixels looks like noise.
...
Same canvas, same price, same currency, same chain, same minimum, same three
fields. This is our product, already live. At the time of writing it had sold
roughly 300 of its million pixels, so the board is effectively empty.
...
**Their published rules commit them to blocks that are permanent and
non-transferable** — bought once, never resold. This is a factual statement
about their product, and it is the single sharpest divergence from ours.
...
1millionpixels.xyz is the same product, live, with the same price, canvas,
minimum, currency, chain, storage provider, and form fields.
...
Our differentiators are the things they ruled out in writing: **each block is an
NFT its owner can resell**, where theirs are non-transferable; and our canvas is
a live product with moderation, where theirs is a fill-once artifact headed for
a single auction. We should not try to out-narrate them. We should out-build
them.
...
| From | What | Why |
| --- | --- | --- |
| original | 10×10 minimum, and the reasoning for it | A 5×5 board is visibly noisier and cheapens the unit; the argument about legibility and clickability still holds |
...
| 1mp | Non-transferable blocks | This is our core differentiator inverted. Our block is an NFT its owner can sell |
...
| mdsp | 5×5 minimum | Noisier board, cheaper unit |
...
13. **"Transferable" becomes headline copy, not a footnote.** The nearest
    competitor commits in writing to blocks that can never be resold. That is
    our positioning handed to us, and the home page should answer it directly.
