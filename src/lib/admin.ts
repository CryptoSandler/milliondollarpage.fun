 ...
 * Admin access.
 *
 * Adapted from pixelwar's `src/lib/admin.ts`, which was shaped by three
 ...
 *
 *     **What that does NOT currently buy, stated because it is easy to read
 *     "revocable" as more than it is:** there is no way to revoke somebody
 *     else's session. `revokeAdminSession` has exactly one caller, `DELETE
 ...
 *     missing piece; it is named here rather than implied.
 ...
 *
 * WHO CALLS THIS: nothing yet. Task 3 of this batch adds the callers — `POST
 * /api/admin/session` signs in and `DELETE` on the same route signs out
 * (`src/app/api/admin/session/route.ts`), and `requireAdmin`
 * (`src/lib/admin-guard.ts`) reaches `authenticateAdmin` on behalf of every
 * takedown route. `adminSessionLabel` is for the `/admin` page, which is Task
 * 4. Said plainly rather than dressed up as existing callers: per CLAUDE.md a
 * module whose header cannot name a caller is a module nobody asked for, and
 * the honest answer here is that the callers are one task away, not that they
 * are already there.
 */

export const ADMIN_COOKIE = "mdp_admin";
 ...
 * is the only form this project reads. The shape below — a list, and a label
 * on each entry — is what makes adding a second operator a change to this one
 * function and not to the schema.
 */
 ...
 *
 * Fails closed, because `clientIp` does (`callers/client-ip.ts`): with no
 * trustworthy header it returns a reason rather than a shared bucket, and this
 * function passes that refusal straight through. A shared bucket matters more
 * here than anywhere else on the site — it is shared by everyone we cannot
 * identify, so the first anonymous caller to burn five guesses locks the
 * operator out of their own admin surface, and a lockout on a shared key is a
 * denial of service against the person it was built to protect. The reason
 * names the environment variable that switches address trust off, so it goes
 * to the server log and never to the caller (the same split `identify` makes
 * in `http.ts`).
 *
 ...
 * not refuse — it returns the literal `untrusted-local` for every caller, so
 * this function succeeds and hands back the one hash they all share. That is
 * exactly the shared bucket described above, reinstated: under that flag
 * anybody can spend the operator's five guesses. The flag is
 * development-only, `.env.example` says it must stay unset in production, and
 * `assertUntrustedClientIpNotInProduction` fails startup if it is set there;
 * this note is here so nobody reads "fails closed" as "cannot share a bucket".
 */
 ...
 * rows a month.
 *
 * The `admin_sessions_live (expires_at, revoked_at)` index from migration 009
...
/**
 * For server components — the `/admin` page Task 4 adds. Reads the cookie
