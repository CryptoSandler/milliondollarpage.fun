...
/**
 * Admin access, adapted from pixelwar along with the module. Four properties
 * are what this file is here to hold down: the cookie carries a revocable
 * session id and not the secret, failed logins are counted and locked out, the
 * token comparison leaks neither the secret's length nor which token matched,
 * and an unset `ADMIN_TOKEN` means REFUSE rather than open.
 *
 * Every assertion reads what the code and the DATABASE actually did. Nothing
 * here restates a constant back at itself: the lockout tests drive
 * `recordLoginAttempt` the configured number of times and then ask the gate,
 * so they fail if the counter stops counting — a test that asserted
 * `maxFailures === 5` would pass against a gate that had been deleted.
 *
...
beforeEach(() => {
  ...
  // The suite deletes ALLOW_UNTRUSTED_CLIENT_IP so the strict address path is
  // the one under test; pin the hop count so a developer's .env.local cannot
  // change what `request()` above means.
...
describe("token identification", () => {
  ...
  it("does not leak the secret's length through an early return", { timeout: 20_000 }, () => {
    ...
    // None of these may throw: timingSafeEqual on mismatched lengths does, so
    // a comparison rewritten over the raw strings turns these into errors
    // rather than nulls — which is the leak, one guess at a time.
...
describe("fail closed when ADMIN_TOKEN is unset", () => {
  ...
  it(
    ...
    async () => {
      ...
      // cleared. Unset must mean refuse, not "open to whoever knows it". The
      // first assertion is what makes the second one mean something: it
      // proves this exact request was accepted a moment ago.
...
describe("login lockout", () => {
  ...
  it("clears the streak on a success", { timeout: 20_000 }, async () => {
    ...
    // Eight failures in the window, and no lockout: an operator who mistypes
    // four times, gets in, and mistypes four more is not locked out. Without
    // the streak reset these would add up.
    expect((await checkAdminLoginGate(IP)).ok).toBe(true);
  });
...
describe("sessions", () => {
  ...
  it(
    "can be revoked server-side, so a leaked cookie is not a redeploy",
    { timeout: 20_000 },
    async () => {
      const session = await createAdminSession("admin", IP);
...
describe("authenticating a request", () => {
  ...
  it("refuses a token header from an address it cannot trust", { timeout: 20_000 }, async () => {
    ...
    const bare = new Request("https://milliondollarpage.fun/api/admin/takedowns", {
...
/**
 * Both admin tables gain a row per sign-in attempt and have no sweep of their
 * own, so without this they grow forever — slowly, which is exactly why
 * nothing would notice.
 *
 * This drives `recordLoginAttempt`, the caller, rather than the sweep itself:
 * a sweep nothing runs is a sweep that does not exist. Falsify it by deleting
 * the `pruneAdminRecords()` call from `recordLoginAttempt`.
 */
describe("pruning the admin tables", () => {
  it(
    ...
    async () => {
      ...
      await query(
        `UPDATE admin_sessions SET expires_at = now() - interval '60 days' WHERE id = $1`,
        [dead.id],
      );
      ...
      expect(await query<{ id: string }>(`SELECT id FROM admin_sessions`)).toEqual([
        { id: live.id },
      ]);
