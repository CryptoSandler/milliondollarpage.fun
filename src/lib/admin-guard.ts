 ...
 *
 * Deliberately NOT the sign-in route's answer for an unconfigured deployment.
 * That route has to tell an operator standing in front of it why the form does
 * not work; these routes have no such duty, and "this deployment has no admin
 * surface" is exactly the kind of thing a prober is trying to establish. The
 * operator learns it from the server log instead, which is where configuration
 * faults belong.
 *
 * Deliberately a separate file from `admin.ts`: that module was adapted line by
 * line from a hardened source and knows nothing about HTTP responses. This is
 * the HTTP shell around it, and keeping them apart is what lets the refusal be
 * one function rather than a convention each route follows.
 *
 * WHO CALLS THIS: nothing yet. Task 3 of this batch adds the callers — `GET
 * /api/admin/takedowns` and the three actions on `POST
 * /api/admin/blocks/[id]`. Every admin route added after them must call it
 * too; that is the point of it being one function.
 */
...
export type AdminGuard = { ok: true; label: string } | { ok: false; response: Response };
