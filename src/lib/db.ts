 ...
 * interpolated into SQL. The one guarantee milliondollarpage.fun cannot ever
 * break is that no two blocks are ever sold to occupy the same pixel, and
 * that promise is a Postgres constraint rather than application logic, which
 * is only global when every request talks to the same database instance
 * rather than a per-process store.
 */
