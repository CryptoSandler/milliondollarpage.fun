...
```bash
EV=~/proyectos/evidencia/milliondollarpage/$(date -u +%Y-%m-%d)-$(git branch --show-current); mkdir -p "$EV"
...
**`milliondollarpage` is 3103** — `npm run dev` and `npm start` carry `-p 3103`, and any
...
command agrees with itself. Measured for this repository on 2026-09-02:
    ...
    SKIPS   a02f928 (.claude/ only)
    BUILDS  8eb0eb6 (package.json, scripts/)
